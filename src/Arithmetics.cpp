...
namespace Mat {
  template<typename T>
  bool add(Matrix<T>& dst, const Matrix<T>& A, const Matrix<T>& B); 
  template<typename T>
  bool subtract(Matrix<T>& dst, const Matrix<T>& A, const Matrix<T>& B);
  template<typename T> 
  bool scale(Matrix<T> &dst, const T& scalar);
}

template<typename T>
bool Mat::add(Matrix<T>& dst, const Matrix<T>& A, const Matrix<T>& B) {  
...
template<typename T> 
bool Mat::subtract(Matrix<T>& dst, const Matrix<T>& A, const Matrix<T> &B) { 
  if (A.numRows() == 0 || A.numCols() == 0) return false;
